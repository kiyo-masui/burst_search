...
void dedisperse_blocked_cached_inplace(float **dat, int nchan, int ndat, int nchan1, int chunk_size){
  ...
  float **tmp1,**tmp2;
  if (nchan1>nchan2) {    
    tmp1=matrix(nchan1,chunk_size+nchan1);
    //tmp2=matrix(nchan1,chunk_size+nchan1); 
  }
  else {
    tmp1=matrix(nchan2,chunk_size+nchan2);
    //tmp2=matrix(nchan2,chunk_size+nchan2); 
  }
  ...
  for (int i=0;i<nblock;i++) 
    ...
    for (int i=0;i<nblock;i++) {
      ...
      for (int j=0;j<nchunk;j++) {
        ...
        //printf("dedispersing %i\n",j);
        //dedisperse(tmp1,tmp2,nchan1,chunk_size+nchan1);
        dedisperse_inplace(tmp1,nchan1,chunk_size + nchan1);
        for (int k=0;k<nchan1;k++)
          memcpy(&(dat[i*nchan1+k][istart]),tmp1[k],chunk_size*sizeof(float));
      }
    }
  #endif
  #if 1
    ...
    for (int i=0;i<nblock;i++) 
      for (int j=0;j<nchan1;j++)
        //not aligned
        dat[j*nblock + i] = zero + (i*nchan1 + j)*ndat + i*j;
        //memcpy(dat[j*nblock+i],dat2[i*nchan1+j]+i*j,ndat-i*j);
  #endif


  #if 0
    for (int i=0;i<nblock2;i++)
      dedisperse(dat+i*nchan2,dat2+i*nchan2,nchan2,ndat);
  #else
    for (int i=0;i<nblock2;i++) {
      ...
      for (int j=0;j<nchunk;j++) {
        ...
        for (int k=0;k<nchan2;k++)
          memcpy(tmp1[k],dat[i*nchan2+k]+istart,(istop-istart)*sizeof(float));

        //printf("dedispersing 1 %i %i\n",i,j);
        //dedisperse(tmp1,tmp2,nchan2,chunk_size+nchan2);
        dedisperse_inplace(tmp1,nchan2,chunk_size+nchan2);
        //printf("ping\n");
        //dedisperse_inplace(tmp1,nchan2,chunk_size + nchan2);

        for (int k=0;k<nchan2;k++)
        {
          //printf("copying channel %i from %i to %i\n",i*nchan2 + k,istart,istart + chunk_size*sizeof(float));
          //printf("inds %i to %i\n",(i*nchan2 + k)*(327688) +istart,(i*nchan2 + k)*(327688) + istart + chunk_size*sizeof(float));
          memcpy(&(dat[i*nchan2+k][istart]),tmp1[k],chunk_size*sizeof(float));
        }
      }
    }
  #endif
